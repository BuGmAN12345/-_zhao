...
def watermark_image(img, wm):
	...
	if(c1*c2//(bs*bs) < w1*w2*redundancy):
	...
	while(i < blocks_needed):				
		...
		for _ in range(redundancy):
			while(True):
				x = random.randint(1, blocks) #保证每个x都不重复
				if(x not in st):
					st.add(x)
					break
			# n = c1//bs
	...
	cv.imwrite("tmp_image/"+watermarked_img , final)
	return imf
...
def extract_watermark(img, ext_name):
	...
	wm = [[0 for x in range(w2)] for y in range(w1)]
	st = set()
	random.seed(key)
	i = 0
	cnt = 0
	...
	print(wm)
	"""
	cv.imwrite("tmp_image/"+ext_name , wm)
...
def Translate(img, dx=50, dy=30):
    # dx: 水平方向偏移量，dy: 垂直方向偏移量
    rows, cols = img.shape[:2]
    M = np.float32([[1, 0, dx], [0, 1, dy]])
    shifted = cv.warpAffine(img, M, (cols, rows))
    return shifted

def AdjustContrast(img, alpha=1.5, beta=0):
    """
    alpha: 对比度因子，>1 增强对比度，<1 降低
    beta: 亮度偏移（可选），默认不变
    """
...
if __name__ == "__main__":
	...
	# cv.imshow('Watermarked Image', wmed)
	print("\nWatermarking Done!\n")
	...
	wx = cv.imread("tmp_image/"+"Extracted_SigAtt_GaussNoise.jpg", 0)
	print("NCC:", NCC(wm, wx))
	...
	print("\nChecking when image is flip horizontal:")
	x = HorizontalFlip(wmed)
	wx = extract_watermark(x, "Extracted_flip_horizontal.jpg")
	print("NCC:", NCC(wm, wx))

	print("\nChecking when image is translated by 50 pixels in x and 30 pixels in y:")
	x = Translate(wmed)
	wx = extract_watermark(x, "Extracted_translate.jpg")
	print("NCC:", NCC(wm, wx))

	print("\nChecking when contrast is adjusted")
	x = AdjustContrast(wmed)
	wx = extract_watermark(x, "Extracted_adjust_contrast.jpg")
	print("NCC:", NCC(wm, wx))